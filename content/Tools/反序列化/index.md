...
序列化是把内存里的对象转成可储存、可传输的字节流或字符串；反序列化时反过来，根据这船数据把对象重建出来。这本身是个正常的功能，用于跨进程、跨网络传递对象状态。

反序列化不是被动地读数据，而是主动地重建对象。在重建地过程中，程序会自动实例化数据里指定地类、填充它的属性，而且很多语言会在这个过程里自动调用一些特殊方法（魔术方法/回调），比如 PHP 地 `__wakeup`、`destruct`，Java 的 `readObject`。

如果攻击者能控制被反序列化的数据，他就控制了哪个类被实例化、属性时什么值。于是他可以挑选程序里已有的、危险的类，拼装处一条从魔术方法触发、一步步调用到危险操作（如执行命令、写文件）的调用链，这就是 gadget chain（利用链/POP 链）。攻击者没有注入新代码，而是把目标程序里已经存在的代码片段当积木重新编排，最后拼处 RCE。

## PHP 反序列化

### 入口与魔术方法

入口是 `unserialize()` 处理了用户可控数据。触发链靠魔术方法，按什么时候被调用分为：

- `__wakeup()`：反序列化时自动调用
- `__destruct()`：对象被销毁时调用（脚本结束时必触发，是常用的链起点）
- `__toString()`：对象当字符串使用
- `__get()`、`__set()`、`__call()`：访问不存在的属性/方法时

### POP 链（面向属性编程）

单个魔术方法往往不能达成目的，以某个魔术方法为起点，精心构造的对象属性，让它去调用另一个对象方法，层层传递，最终达到一个能执行命令或写文件的危险终点。比如 `__destruct` 里调用了 `$this -> obj -> close()`，而把 `obj` 控制成另一个 `close` 里有危险操作的类。

### 序列化格式可篡改

PHP 序列化是明文可读的，`O:4:"User":2:{s:4:"name";s:5:"admin";...}`。`O` 是对象、`4` 是类名长度、`2` 是属性个数。攻击者可以直接修改属性值、类型、甚至属性个数。
