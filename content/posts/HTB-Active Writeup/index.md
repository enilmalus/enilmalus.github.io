...
根据 Nmap 的扫描信息，靶机为 `Windows Server 2018 R2SP1`，为域控制器，域名为 `active.htb`，主机名为 `DC`。
...
靶机开放 139 和 445 端口，存在 SMB，执行 smbmap 扫描。
...
发现一个 `READ ONLY` 共享目录 `Replication`，使用 smbclient 再次确认。
...
这是一个 GPP 加密，使用 gpp-decrypt 进行解密得到 `SVC_TSG` 的密码为 `GPPstillStandingStrong2k18`。
...
使用 `SVC_TGS` 枚举 SMB，发现共享目录 `Users`。
...
使用 psexec 登陆 `Administator`，得到 `root.txt`。
